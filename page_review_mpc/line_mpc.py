...
class SplineSegment:
    def __init__(self, waypoints, start_tangent=None, end_tangent=None):
        ...
        self.arc_length_params = np.concatenate(([0], np.cumsum(distances)))
        self.length = self.arc_length_params[-1]
        
        self.t_params = self.arc_length_params / self.length if self.length > 1e-6 else np.linspace(0, 1, len(self.waypoints))
...
# [SỬA ĐỔI] Sửa lại logic thêm mốc để đảm bảo mọi điểm nối đều được ghi nhận
# ==============================================================================
...
class PathManager:
    def __init__(self, num_points_per_meter=20):
        ...
        # [SỬA ĐỔI] Khởi tạo với danh sách rỗng, sẽ thêm mốc 0 trong add_segment
        self.milestone_s_values = []
        self.discretized_path = None
        self.num_points_per_meter = num_points_per_meter

    def add_segment(self, segment):
        # [SỬA ĐỔI] Xử lý trường hợp thêm segment đầu tiên
        if not self.segments:
            # Nếu đây là segment đầu tiên, thêm điểm bắt đầu (s=0) làm mốc đầu tiên
            self.milestone_s_values.append(0.0)

        current_s_start = self.total_length
        
        self.segments.append(segment)
        self.total_length += segment.length
        self.cumulative_lengths.append(self.total_length)

        # [SỬA ĐỔI] Logic thêm mốc được làm rõ ràng hơn
        if isinstance(segment, SplineSegment):
            # Với Spline: xóa mốc nối (là điểm cuối của segment cũ)
            # và thay thế bằng chuỗi mốc chi tiết của spline.
            if len(self.milestone_s_values) > 0:
                 self.milestone_s_values.pop()
            
            # Thêm tất cả các waypoint của spline làm mốc, tính theo s toàn cục
            for s_local in segment.arc_length_params:
                self.milestone_s_values.append(current_s_start + s_local)
        else:
            # Với Line và Arc: chỉ cần thêm điểm cuối của chúng làm mốc.
            # Điểm bắt đầu của chúng đã là mốc cuối của segment trước đó rồi.
            self.milestone_s_values.append(self.total_length)
    ...
    def clear(self):
        ...
        self.total_length = 0.0
        self.cumulative_lengths = [0.0]
        # [SỬA ĐỔI] Reset lại danh sách mốc
        self.milestone_s_values = []
        self.discretized_path = None
    ...
    def generate_example_path(self, start_pose):
        ...
        self._discretize_path()
        rospy.loginfo(f"Generated example path with {len(self.segments)} segments and {len(self.milestone_s_values)} milestones. Total length: {self.total_length:.2f}m")
        return True
...
class MPCController:
    def __init__(self, dt, v_max, v_min, omega_max, omega_min, lookahead_min, lookahead_max, curvature_threshold, factor_change_rate):
        ...
        self.path_milestones_coords = [] # Chỉ để vẽ, không dùng cho logic điều khiển
    ...
    def mpc_control(self, x, y, theta):
        ...
        if self.stepping_mode:
            if self.current_milestone_target_index <= 0 or self.current_milestone_target_index >= len(self.path_manager.milestone_s_values):
                rospy.logwarn("Invalid milestone target index.")
                self.reached_goal = True
                return 0.0, 0.0
            target_s = self.path_manager.milestone_s_values[self.current_milestone_target_index]
        ...
        # Cập nhật ramp_down_distance để phù hợp với từng chặng stepping
        if self.stepping_mode:
            s_start_of_leg = self.path_manager.milestone_s_values[self.current_milestone_target_index - 1]
            leg_length = target_s - s_start_of_leg
            ramp_down_dist = min(self.ramp_down_distance, leg_length * 0.5)
        else:
            ramp_down_dist = self.ramp_down_distance

        if remaining_distance < ramp_down_dist:
             v *= np.sin((remaining_distance / ramp_down_dist) * np.pi / 2)
...
class GUI:
    def __init__(self, master, mpc_controller):
        ...
        self.milestone_plot, = self.ax_trajectory.plot([], [], 'go', label="Mốc (Milestones)", markersize=7, linestyle='None', zorder=4)
    ...
    def _prepare_path_visualization(self):
        ...
        self.mpc_controller.path_milestones_coords.clear()
        
        if self.mpc_controller.path_manager.milestone_s_values:
            points = [self.mpc_controller.path_manager.get_point_at_global_s(s) 
                      for s in self.mpc_controller.path_manager.milestone_s_values]
            self.mpc_controller.path_milestones_coords.extend(points)
        
        if self.mpc_controller.path_milestones_coords:
            milestone_points = np.array(self.mpc_controller.path_milestones_coords)
    ...
    def step_to_next_milestone(self):
        ...
        if self.mpc_controller.current_milestone_target_index >= len(self.mpc_controller.path_manager.milestone_s_values):
            self.result_label.config(text="End of path reached. Resetting.")
            self.clear_path()
            return

        target_dist = self.mpc_controller.path_manager.milestone_s_values[self.mpc_controller.current_milestone_target_index]
