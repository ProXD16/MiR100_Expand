...
class PathManager:
    ...
    def generate_example_path(self, start_pose):
        """
        Tạo một quỹ đạo chỉ bao gồm MỘT cung tròn duy nhất.
        Quỹ đạo sẽ bắt đầu tiếp tuyến với vị trí và hướng hiện tại của robot.
        """
        # Xóa quỹ đạo cũ trước khi tạo quỹ đạo mới
        self.clear()
        
        # 1. Lấy vị trí (x, y) và hướng (theta) ban đầu của robot
        p0 = start_pose[:2]
        theta0 = start_pose[2]
        
        # 2. Định nghĩa các tham số cho cung tròn
        radius = 1.2          # Bán kính 1.2m theo yêu cầu
        angle_sweep = np.pi   # Quét một góc 180 độ (pi radians). Bạn có thể thay đổi giá trị này.
        clockwise = True     # Di chuyển ngược chiều kim đồng hồ (rẽ trái). Đặt thành True để rẽ phải.

        # 3. Tính toán tâm của cung tròn để đảm bảo quỹ đạo tiếp tuyến với hướng robot
        #    - Vector chỉ đến tâm của vòng tròn sẽ vuông góc với hướng đi của robot.
        #    - Đối với rẽ trái (ngược chiều kim đồng hồ), tâm sẽ nằm bên trái hướng đi.
        if clockwise:
            # Rẽ phải: vector tâm nằm bên phải so với hướng đi
            center_offset_vector = np.array([np.sin(theta0), -np.cos(theta0)])
        else:
            # Rẽ trái: vector tâm nằm bên trái so với hướng đi
            center_offset_vector = np.array([-np.sin(theta0), np.cos(theta0)])
            
        center = p0 + radius * center_offset_vector
        
        # 4. Tính toán góc bắt đầu và kết thúc của cung tròn
        #    - Góc bắt đầu là góc của vector nối từ tâm (center) đến điểm bắt đầu (p0).
        start_angle = np.arctan2(p0[1] - center[1], p0[0] - center[0])
        
        #    - Góc kết thúc được tính dựa trên góc bắt đầu và góc quét.
        if clockwise:
            end_angle = start_angle - angle_sweep
        else:
            end_angle = start_angle + angle_sweep
        
        # 5. Tạo và thêm phân đoạn cung tròn duy nhất vào bộ quản lý đường đi
        arc_seg = ArcSegment(center, radius, start_angle, end_angle, clockwise=clockwise)
        self.add_segment(arc_seg)
        
        # 6. Rời rạc hóa quỹ đạo để chuẩn bị cho việc bám và hiển thị
        self._discretize_path()
        
        # Ghi log để xác nhận quỹ đạo đã được tạo
        direction_str = "theo chiều kim đồng hồ" if clockwise else "ngược chiều kim đồng hồ"
        rospy.loginfo(f"Đã tạo quỹ đạo cung tròn {np.rad2deg(angle_sweep):.0f} độ ({direction_str}). Bán kính: {radius}m. Dài: {self.total_length:.2f}m")
