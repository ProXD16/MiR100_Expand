...
# [SỬA ĐỔI] Thay đổi logic tạo đường đi để chỉ tạo một đường thẳng 3m
# ==============================================================================
...
class PathManager:
    ...
    # --- [SỬA ĐỔI] ---
    # Chức năng này bây giờ chỉ tạo một đường thẳng dài 3 mét.
    def generate_example_path(self, start_pose):
        self.clear()
        
        # Lấy vị trí và hướng ban đầu của robot
        p0 = start_pose[:2]
        theta0 = start_pose[2]
        
        # Tạo một đoạn thẳng dài 3 mét từ vị trí ban đầu theo hướng của robot
        line_seg = LineSegment(p0, p0 + 3.0 * np.array([np.cos(theta0), np.sin(theta0)]))
        
        # Thêm đoạn thẳng này vào đường đi
        self.add_segment(line_seg)
        
        # Rời rạc hóa đường đi để trực quan hóa và tìm điểm gần nhất
        self._discretize_path()
        
        # Ghi log thông báo
        rospy.loginfo(f"Generated a 3m straight line path. Total length: {self.total_length:.2f}m")
        return True
    # --- [KẾT THÚC SỬA ĐỔI] ---
